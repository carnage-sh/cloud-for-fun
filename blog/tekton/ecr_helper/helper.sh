...
#    command line to select namespace and (optionally) tekton service
#    account name.
#
...
# secret with the appropriate metadata for usage Tekton task runs,
# accessible by a service account named "tekton" (by default).
...
EXPIRES_AT=$(echo $DATA | jq -r '.authorizationData[0].expiresAt | trunc')
